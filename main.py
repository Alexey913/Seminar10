...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        'Для начала работы выбери пункт меню или введите команду\nКалькулятор\nВывод логов\nВыход',
        reply_markup=markup_start)
    return main_menu
...
async def int_num_1(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text(
            f'{chr(128683)} Ошибка, допускается ввод только чисел, пожалуйста повторите ввод:')
        return int_num_one
...
async def complex_num_3(update: Update, context: ContextTypes.DEFAULT_TYPE) -> float:
    global y_1
    try:
        y_1 = complex(update.message.text)
...
async def float_div(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Продожить\nНовый ввод\nГлавное меню\nВыход', \
        ...
        await update.message.reply_text(f'{chr(128683)} Ошибка, на ноль делить нельзя, выберите в меню другое действие '
                                        f'с числами {x} и {y}')
        return action_menu
...
async def floor_div(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Продожить\nНовый ввод\nГлавное меню\nВыход', \
        ...
        await update.message.reply_text(f'{chr(128683)} Ошибка, целочисленное деление не применимо к комплексным числам,'
                                        f'попробуйте другую операцию ')
        return action_menu
...
async def mod_div(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Продожить\nНовый ввод\nГлавное меню\nВыход', \
        ...
        await update.message.reply_text(f'{chr(128683)} Ошибка, деление по модулю не применимо к комплексным числам,'
                                        f'попробуйте другую операцию ')
        return action_menu
